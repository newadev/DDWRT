# DDWRT (OpenWrt) 固件全自动编译库

利用 GitHub Actions，基于设备的完全隔离架构，实现在线编译、自动打包、每周自动检测上游源码并构建你专属的 OpenWrt 路由固件。

---

## 🚀 核心优势：为什么选择这个架构？

- **免维护、自动更新**：系统每周五会自动对比 OpenWrt 官方最新源码，一旦有修改便自动触发编译并发布到 Releases。旧产物只保留7天，永不爆仓。
...
1. 在 `Actions` 页面左侧点击 `Build OpenWrt`。
